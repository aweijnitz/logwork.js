...
var distributeHours = function distributeHours(nrTickets, hours) {
  if (nrTickets == 1)
    ...
    return [part,
      distributeHours(nrTickets.slice(1), (hours - part))
    ];
  }
};
...
var logwork = function logwork(jiraTicket, moreTickets, hours, commentsFile) {
  var tickets = [];
  var comments = [];
  ...
  if (!!moreTickets) {
    moreTickets.forEach(function (ticket) {
      tickets.push(ticket);
    });
  }

  comments = shuffle(getComments(commentsFile));
  var hoursDistributed = flattenArrayOfArrays(distributeHours(tickets.length, hours));

  // for each ticket, execure jira command
  tickets.forEach(function (ticket, index) {
    var child;
    child = exec("jira workadd " + ticket + " " + comments(index % comments.length),
